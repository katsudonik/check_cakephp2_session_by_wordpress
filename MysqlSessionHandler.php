...
$env = 'development';
require_once "/www/htdocs/app/Config/database_{$env}.php";
$settings = new DATABASE_CONFIG();
$this->_host = $settings->default['host'];
$this->_user = $settings->default['login'];
$this->_pass = $settings->default['password'];
$this->_db = $settings->default['database'];
   }
    /**
...
$id = $_COOKIE['CAKEPHP'];
        $sql = "SELECT * FROM cake_sessions WHERE id LIKE '${id}'" ;
    ...
    function write($id, $data)
    {
        ...
        $sql = "REPLACE INTO cake_sessions VALUES ('${id}', '${data}', '${date}')" ;
    ...
    {
        $id = mysqli_real_escape_string($this->_link, $id);
        $sql = "DELETE FROM cake_sessions WHERE id LIKE '${id}'" ;
        mysqli_query($this->_link, $sql);
        return true;
    }
    /**
    ...
    function gc($maxlifetime)
    {
        ...
        $sql = "DELETE FROM cake_sessions WHERE (TIMESTAMP(CURRENT_TIMESTAMP) - TIMESTAMP(created)) > ${maxlifetime}" ;
        mysqli_query($this->_link, $sql);
        return true;
    }
}
